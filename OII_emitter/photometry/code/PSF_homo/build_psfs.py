...
class create_single_psf():
    ...
    def __init__(self, image_dir, image_list, filtername, gaia_coords, gaia_cat, ext=1): #ext=0

        self.image_dir = image_dir
        self.image_list = image_list

        #fitsfile = fits.open(glob(self.image_dir + '*' + filtername + '*sci.fits')[0])[ext] change by Lu.
        fitsfile = fits.open(self.image_list[0])[ext]
    ...
    def cutouts(self, cutout_size, plot=True, messages=True,offset_lim=5):
        ...
        for j, (x, y) in enumerate(zip(self.xpix, self.ypix)):
            ...
            if (x1 > 0 and x2 < xmax and y1 > 0 and y2 < ymax):
                ...
                #new_cutout_badastro = self.image[x1 - 10:x2 + 10, y1 - 10:y2 + 10]
                ...
                plt.savefig(f'/home/lupengjun/OII_emitter/photometry/output/psf_gaia/star_cutout/{self.filtername}_{j}_cutout.png')
                plt.clf() #add by Lu
                plt.close() #add by Lu
                ...
                # Subtract background
                aper_annulus = CircularAnnulus((Z, Z), r_in=nc_shape[0]/2-3, r_out=nc_shape[0]/2) #change by Lu
                phot = aperture_photometry(new_cutout, aper_annulus)
                bkg_mean = phot['aperture_sum'][0] / aper_annulus.area
                if messages:
                    print("bkg mean: %.9f"%bkg_mean)
                ...
                if plot:
                    ...
                    plt.savefig(f'/home/lupengjun/OII_emitter/photometry/output/psf_gaia/star_cutout/test_{self.filtername}_{j}_cutout_final.pdf')
                    plt.clf()
                    plt.close()
    ...
    def sum_med_psf(self, exclusion_list=None):
        ...
        sum_psf = np.sum([yy for xx, yy in zip(self.valid_indices, self.star_cutouts) if xx not in exclusion_list], axis=0)
        med_psf = np.median([yy for xx, yy in zip(self.valid_indices, self.star_cutouts) if xx not in exclusion_list], axis=0)

        valid_list = [xx for xx in self.valid_indices if xx not in exclusion_list]
        print('------------> Choose index:', valid_list)
        ...
        new_sum_psf = np.roll(sum_psf, int(xdiff_sum), axis=0) #change int() to round(), by Lu
        ...
        new_med_psf = np.roll(med_psf, int(xdiff_med), axis=0)
        new_med_psf = np.roll(new_med_psf, int(ydiff_med), axis=1)
        ...
        plt.savefig(f'/home/lupengjun/OII_emitter/photometry/output/psf_gaia/star_cutout/stack_{self.filtername}_psf_final.pdf')
        plt.clf()
        plt.close()
...
class build_psfs():
    ...
    def __init__(self, fieldname, images_dir, images_band, cutout_size=101,suffix='NIRCAM',offset_lim=5,fov_arcmin=4, ext=0,filt=None,image_list=None,center=None,maglim=25):
        '''
        Args:
        ...
        for band in self.image_band:
            self.empirical_psf(band, offset_lim)

    def query_gaia(self, fov_arcmin=4, maglim=25):
        Gaia.ROW_LIMIT = -1 #add by Lu
        ...
        for s, star in enumerate(reduced_query):
            if star['phot_g_mean_mag'] > maglim:
                ...
                # if is_in_pointing[0] < size_lim + 11\
                #         or is_in_pointing[0] > im_limits[0] - size_lim - 11\
                #         or is_in_pointing[1] < size_lim + 11\
                #         or is_in_pointing[1] > im_limits[1] - size_lim - 11:
                    # print("Gaia exception: number %s has pixel coord %.2f, %.2f"%(s, is_in_pointing[0], is_in_pointing[1]))
                    # print(is_in_pointing[1], size_lim + 11)
                    # print(is_in_pointing[1], im_limits[0] - size_lim - 11)
                    # print(is_in_pointing[0], size_lim + 11)
                    # print(is_in_pointing[0], im_limits[1] - size_lim - 11)
                    continue
                else:
            ...
            for s, star in enumerate(viz_query):
                ...
                #         or is_in_pointing[1] < size_lim + 11 \
                #         or is_in_pointing[1] > im_limits[1] - size_lim - 11:
                    print("Vizier exception: number %s has pixel coord %.2f, %.2f"%(s, is_in_pointing[0], is_in_pointing[1]))
                    pass
                else:
                    ra = coord.ra.to_string(unit=u.hourangle, sep=':')
                    dec = coord.dec.to_string(unit=u.degree, sep=':')
    ...
    def empirical_psf(self, filtername, offset_lim):
        """
        im: image name, e.g. 'CEERS_HDR-f606w_sci.fits'
        update: image name, e.g. 'egs_all_wfc3_ir_f140w_030mas_v1.9_nircam1_mef.fits.gz' by Lu.
        offset_lim: 
        """

        # filtername = 'F' + im.split('_f')[1].split('_')[0]
        # filtername = filtername.upper()

        self.psf[filtername] = create_single_psf(self.image_dir, self.image_list, filtername.lower(), self.gaia_coords, self.gaia_cat, ext = self.ext)
        ...
        self.working_psf_sum = working_psf_sum
        self.working_psf_median = working_psf_median
